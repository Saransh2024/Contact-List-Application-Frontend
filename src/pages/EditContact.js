...
const EditContact = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    const res = await fetch(`https://contact-management-system-3muu.onrender.com/api/contact`, {
      method: "PUT",
      headers: {
  ...
  useEffect(async () => {
    setLoading(true);
    try {
      const res = await fetch(`https://contact-management-system-3muu.onrender.com/api/contact/${id}`, {
        method: "GET",
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
